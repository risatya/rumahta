	...
	public function invoice($token){
		if($this->session->userdata('logged_in') == false){
			...
			if($cek){
				...
				$admin_list = $this->mdl_home->getAllAdmin();
				
				$this->load->library('invoice');
				foreach($admin_list as $admindata):
					$emailconfig = array(
						'sendto' => $admindata->email,
						'order_from' => $this->session->userdata('username'),
						'order_paket' => $data['paket'][0]->nama_paket
					);
					$this->invoice->invoice_pesan_paket_for_admin($emailconfig);
				endforeach;
		...
		if($this->session->userdata('logged_in') == false){
			...
			if ($this->form_validation->run() == FALSE){
				...
				if($cek){
					...
					//kirim email notifikasi ke admin
					$admin_list = $this->mdl_home->getAllAdmin();
					$data['paket'] = $this->mdl_paket_listing->getPaketDetail($info_paket[0]->id_info_paket);
					...
					foreach($admin_list as $admindata):
						...
						$this->invoice->invoice_konfirmasi_paket_for_admin($emailconfig);
					endforeach;
