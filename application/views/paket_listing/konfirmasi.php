					...
					<div id="inside_main_wrapper">
						...
						<legend>Konfirmasi Pembayaran</legend>
							...
							<table class="table table-bordered table-stripped">
								...
								<tbody>
									...
									<tr>
										...
										<td><?php echo $item->maks_listing." Listing"; ?></td>
										...
										<td>
											<?php echo anchor("#konfirmasi".$item->id_paket_dummy,"<i class='icon icon-share-alt'></i> Konfirmasi",array("class"=>"btn btn-small","data-toggle"=>"modal","title"=>"Konfirmasi Pembayaran")); ?>
											<?php echo anchor("#konfirmasi_batal".$item->id_paket_dummy,"<i class='icon icon-remove'></i>",array("class"=>"btn btn-small","data-toggle"=>"modal","style"=>"width:12px;","title"=>"Batalkan pemesanan")); ?>
										</td>
									</tr>
									<?php $no++; ?>
									<?php endforeach;?>
							...
							<?php if($no <= 1){ ?>
								<div class="alert alert-info">
									Tidak ada paket yang sedang dipesan. Silakan pesan paket listing yang anda inginkan terlebih dahulu.
									<?php echo anchor("paket_listing/index","Pesan paket listing disini",array("class"=>"btn")) ?>
								</div>
							<?php } ?>
					...
					<script type="text/javascript">
						<?php foreach($konfirmasi as $item): ?>
							$('#konfirmasi<?php echo $item->id_paket_dummy; ?>').modal(options);
							$('#konfirmasi_batal<?php echo $item->id_paket_dummy; ?>').modal(options);
						<?php endforeach;?>
					</script>
					...
					<?php foreach($konfirmasi as $item): ?>
					<div id="konfirmasi<?php echo $item->id_paket_dummy; ?>" class="modal hide fade">
						<div class="modal-header">
						  <button type="button" class="close" data-dismiss="modal">×</button>
						  <h3>Konfirmasi Pembayaran <?php echo $item->nama_paket; ?></h3>
						</div>
						...
						<div class="modal-body">
							<?php echo form_open("paket_listing/do_confirm/".$item->id_paket_dummy,array("class"=>"form-horizontal","name"=>"form")); ?>
					...
					<div id="konfirmasi_batal<?php echo $item->id_paket_dummy; ?>" class="modal hide fade">
						<div class="modal-header">
						  <button type="button" class="close" data-dismiss="modal">×</button>
						  <h3>Konfirmasi</h3>
						</div>
						<div class="modal-body">
							Anda Yakin ingin membatalkan pemesanan paket ini ? <br/><br/>
						</div>
						<div class="modal-footer">
							<?php echo anchor("paket_listing/cancel/".$item->id_paket_dummy,"Batalkan Pemesanan",array("class"=>"btn btn-primary btn-large")); ?>
							<button class="btn btn-large" data-dismiss="modal" aria-hidden="true">Cancel</button>
							<?php echo form_close(); ?>
						</div>
					</div>
