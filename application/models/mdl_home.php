...
Class Mdl_home extends CI_Model{
	...
	function getCategoryByIDCategory($id_category){
		return $this->db->get_where('tbl_kategori',array('id_kategori'=>$id_category))->result();
	}
	...
	function select($table,$param1){
		return $this->db->get_where($table,$param1)->result();
	}
	function update($field,$param1,$table,$param2){
		$this->db->where($field,$param1);
		$this->db->update($table,$param2);
	}
	...
	function getFreeQuota(){
		return $this->db->get_where('tbl_info_paket',array('id_info_paket' => 1))->result();
	}
}
